...
def deconv_acc_inst(self, trace, output):
    ...
    if 'VEL' in self.deconv:
        ...
        trace.data = differentiate(improved_integration(trace))
...
#     :param obspy.core.trace.Trace trace: the trace object instance to deconvolve
#     '''
#     print("Trace length: ", len(trace.data))
#     if self.deconv not in 'CHAN':
#         trace.remove_response(inventory=rs.inv, pre_filt=[0.1, 0.6, 0.95*self.sps, self.sps],
#                                 output=output, water_level=4.5, taper=False)
#     else:
...
def msg_process(maxList):
    ...
    :return: the ``ALARM`` message, ready to be put on the queue
    '''
    return b'PROCESS %s %s' % (bytes(str(maxList[0]), 'utf-8'), bytes(str(maxList[1]), 'utf-8'))

def get_msg_process_values(msg):
    '''
    This function gets the path from ``PROCESS`` messages as a string.
    ...
    :return: the path embedded in the message
    '''
    return msg.decode('utf-8').split(' ')[1], msg.decode('utf-8').split(' ')[2]

def get_msg_process_channels(msg):
    '''
    This function gets the path from ``PROCESS`` messages as a string.
    ...
    :return: the path embedded in the message
    '''
    return msg.decode('utf-8').split(' ')[3], msg.decode('utf-8').split(' ')[4]

def get_msg_process_time(msg):
    '''
    This function gets the path from ``PROCESS`` messages as a string.
    ...
    :return: the path embedded in the message
    '''
    return rs.UTCDateTime.strptime(msg.decode('utf-8').split(' ')[5], '%Y-%m-%dT%H:%M:%S.%fZ')
