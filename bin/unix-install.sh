...
if [[ "$arch" == "armv"* ]]; then
  env_install="conda create -n rsudp python=3.6.6 numpy matplotlib future scipy lxml cffi sqlalchemy cryptography -y"
else
  env_install="conda create -n rsudp python=3.7.4 matplotlib=3.1.1 numpy=1.16.4 future scipy lxml sqlalchemy cryptography obspy -y"
fi
