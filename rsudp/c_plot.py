...
class Plot:
    ...
    def _update_specgram(self, i: int, mean: float):
        ...
        self.nlap1 = self.nfft1 * self.per_lap
        if len(self.stream_uf[
                   i].data) < self.nfft1:  # when the number of data points is low, we just need to kind of fake it for a few fractions of a second # Replaced Stream with Stream_uf
            self.nfft1 = 8
            self.nlap1 = 6
        ...
        self.ax[
            i * self.mult + 1].clear()  # incredibly important, otherwise continues to draw over old images (gets exponentially slower)
